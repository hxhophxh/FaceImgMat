...
Write-Host "[提示] 仅下载预编译包（.whl），避免编译错误..." -ForegroundColor Yellow
Write-Host ""

# 下载依赖包（仅下载wheel包，包含所有传递依赖）
python -m pip download -r $requirementsFile -d $packagesDir -i https://pypi.tuna.tsinghua.edu.cn/simple --only-binary=:all: 2>&1 | ForEach-Object {
