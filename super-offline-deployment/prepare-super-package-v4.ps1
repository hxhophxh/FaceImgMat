...
Write-Host "[提示] 优先下载预编译包（.whl）..." -ForegroundColor Yellow
Write-Host ""

# 下载依赖包（优先wheel包，如果没有则允许源码，包含所有传递依赖）
python -m pip download -r $requirementsFile -d $packagesDir -i https://pypi.tuna.tsinghua.edu.cn/simple --prefer-binary 2>&1 | ForEach-Object {
    $line = $_.ToString()
